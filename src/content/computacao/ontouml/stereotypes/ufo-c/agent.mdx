...
`«Agent» Pessoa --inheres in--> «Intentional Moment» Intenção de concluir uma tarefa`
...
Sim. Uma organização pode ser considerada um Agent quando atua, percebe e comunica por meio de agentes internos.

**Exemplo:**

`«Agent» Empresa --internal agent--> «Agent» Representante`

`«Agent» Empresa --creates--> «Event» Emissão de comunicado`
...
### Exemplo 2 — Robô como Agent

`«Agent» Robô`

`«Agent» Robô --perceives--> «Event» Obstáculo detectado`

`«Agent» Robô --creates--> «Event» Desviar de obstáculo`

Robô pode ser modelado como Agent quando possui capacidade de perceber eventos e executar ações.

### Exemplo 3 — Organização como Agent

`«Agent» Organização`

`«Agent» Organização --internal agent--> «Agent» Diretor`

`«Agent» Organização --creates--> «Event» Emissão de decisão`

Organização pode ser modelada como Agent porque atua por meio de agentes internos.
